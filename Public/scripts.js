async function fetchEmpresaByCUIT(cuit) {
  try {
    ...
    if (response.status === 404) {
      // Manejo del caso de CUIT no encontrado
    ...
    if (!response.ok) {
      // Manejo de otros errores de respuesta HTTP
          ...
          <div class="card">
            <div class="card-body">
              ...
              <p class="card-text">Legajo: ${
                empresa.programa.legajo
                  ? empresa.programa.legajo
                  : "No especificada"
              }</p>
              <p class="card-text">Monto: PESOS ${
                empresa.programa.monto
                  ? empresa.programa.monto
                  : "No especificada"
              }</p>
    ...
    const resultadoDiv = document.getElementById("resultado");
    resultadoDiv.innerHTML =
      '<p class="text-danger">Ocurrió un error al obtener los datos de la empresa.</p>';
  }
}
